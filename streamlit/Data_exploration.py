...
from utils import db_query, AIRBORNE_WHERE
...
def table_structure(db_conn):
    ...
    df = (
        db_query(db_conn, "select *", where="altitude != 0", limit=1)
        .transpose()
...
considered.**
    ...
    for column in [
        ...
        df = (
            db_query(db_conn, query, where=AIRBORNE_WHERE)
            .transpose()
    ...
    df = (
        db_query(db_conn, query, where=AIRBORNE_WHERE)
        .transpose()
        .rename(columns={0: "% of valid data"})
    )
    with col2:
